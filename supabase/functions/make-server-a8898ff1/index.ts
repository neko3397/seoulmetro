...
app.get("/make-server-a8898ff1/admin/authorized_employees", async (c: any) => {
  try {
    ...
    if (search) {
      const normalizedSearch = canonicalizeEmployeeName(search);
      query = query.or(`employee_id.ilike.%${search}%,name.ilike.%${search}%,normalized_name.ilike.%${normalizedSearch}%`);
    }
    ...
    console.error("List authorized employees error:", error);
    return c.json({ error: "직원 명단을 조회하지 못했습니다." }, 500);
  }
});
...
app.post("/make-server-a8898ff1/admin/authorized_employees", async (c: any) => {
  try {
    ...
    const { error } = await supabase.from("authorized_employees").upsert({
      employee_id: normalizedEmployeeId,
      name: displayName,
    ...
    console.error("Add authorized employee error:", error);
    return c.json({ error: "직원을 추가하지 못했습니다." }, 500);
  }
});
