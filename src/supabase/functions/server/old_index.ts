...
import { AUTHORIZED_EMPLOYEES } from "./authorized_employees.ts";
const app = new Hono();
// CORS middleware
app.use("*", cors());
...
async function initializeAuthorizedEmployees() {
  try {
    ...
    console.log(`Authorized employees synced (${AUTHORIZED_EMPLOYEES.length} records)`);
  } catch (error) {
      ...
      const defaultCategories = [
        {
          ...
          image: "https://images.unsplash.com/photo-1544620347-c4fd4a3d5957?w=400&h=300&fit=crop",
          description: "지하철 안전 운행을 위한 기초 교육 과정입니다."
        },
        {
          id: "cat_2",
          title: "화재 안전교육",
...
app.post("/make-server-a8898ff1/upload-video", async (c) => {
  try {
    ...
    const { data: signedUrlData, error: signedUrlError } = await supabase.storage.from(bucketName).createSignedUrl(fileName, 60 * 60 * 24 * 365); // 1 year expiry
    if (signedUrlError) {
...
app.post("/make-server-a8898ff1/videos", async (c) => {
  try {
    ...
    const { categoryId, title, description, youtubeId, videoUrl, videoType, duration } = await c.req.json();
    const videosData = await kv.get(`videos_${categoryId}`) || [];
    const newVideo = {
      id: `video_${Date.now()}`,
      title,
    ...
    if (videoType === "youtube" && youtubeId) {
      ...
      newVideo.thumbnail = "https://via.placeholder.com/480x270/4f46e5/ffffff?text=Local+Video";
    } else {
      return c.json({
